...
Store all programming files in 
`C:/Users/[username]/Documents/Programming/`.  
Use the subdirectory `R-D` for main R&D projects.  
The subdirectory `Test` is for temporary files.
...
explain each function, as well as any part of the code that requires 
explanation.  
Comments in C-based languages (ROBOT-C, C++, etc.) start with `//` and span 
one line of code.  
